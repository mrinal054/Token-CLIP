...
#    def load(self, path): # <<<<<<<<<<<<<<<<<<<<<<<<< MKD commented out
#        path = Path(path)
#        assert path.exists()
#        pt = torch.load(str(path))
#        self.load_state_dict(pt)

    def load(self, path): # <<<<<<<<<<<<<<<<<<<<<<<<< MKD added
        path = Path(path)
        assert path.exists()
    ...
    def forward(
        ...
        # enc_image = torch.mean(enc_image, dim=1) # <<<<<<<<<<<<<<<<<<<<<<<< can be improved
   
        # enc_image = enc_image.view(enc_image.shape[0], -1) # <<<<<<<<<<<<<<<<<<<<<<<< can be improved
...
"Example Usage"
# Run it from the following directory: /research/m324371/Project/Digital_Twin/CT-CLIP/CT_CLIP/ct_clip/
if __name__ == "__main__":
    import sys
    from types import SimpleNamespace
    import torch

    sys.path.append("/research/m324371/Project/Digital_Twin/CT-CLIP/scripts/nets/")
    sys.path.append("/research/m324371/Project/Digital_Twin/CT-CLIP/transformer_maskgit/")

    from text_models import build_text_model
    from image_models import build_image_model
    ...
    image_encoder_dict = {
        ...
        "dim_head": 64,
        "heads": 8,
        "classifier_out_chs": None,
        "classifier_dropout": 0.3,
        "classifier_freeze_latents": True,
    }

    device = "cuda" if torch.cuda.is_available() else "cpu"

    # Build encoders
    tokenizer, text_encoder = build_text_model(text_encoder_dict)
    image_encoder = build_image_model(image_encoder_dict)

    # Tokenize batch of 2 strings
    text_data = ["Welcome to Kline's lab!", "We are working on DigitalTwin"]
    tokenizer_encode_kwargs = text_encoder_dict["tokenizer_encode_kwargs"]

    batch = tokenizer(text_data, **tokenizer_encode_kwargs)

    # Move to device
    batch = {k: v.to(device) for k, v in batch.items()}

    # Wrap to match CTCLIP.forward() usage: text.input_ids / text.attention_mask
    text = SimpleNamespace(
        input_ids=batch["input_ids"],
        attention_mask=batch["attention_mask"],
    )

    # Dummy image: (B, C, D, H, W)
    B, C, D, H, W = 2, 1, 64, 256, 256
    image = torch.randn(B, C, D, H, W, device=device)
    ...
    model = CTCLIP(
        ...
        dim_image=512,
        dim_latent=512,
        channels=C,  # IMPORTANT: must match dummy image channels
        use_mlm=False,
        use_visual_ssl=False,
    ).to(device)
    ...
    print("Text encoding shape :", tuple(enc_text.shape)) # Shape: B,L,E (L: No. of tokens, E: Embedding dimension)
    print("Image encoding shape:", tuple(enc_image.shape)) # Shape: B,PD,PH,PW,E (PD: No. of patches along depth axis)
